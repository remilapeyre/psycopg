...
class ReplicationSlotManager:
    ...
    async def create_async_replication_slot(self, acur: AsyncReplicationCursor, name: str, **kwargs):
...
class BaseTest:
    ...
    def _test_extended_protocol(self, conn) -> None:
        ...
        pytest.raises(psycopg.errors.ProtocolViolation, conn.execute, "SELECT %s", params=(1,))
        pytest.raises(psycopg.errors.ProtocolViolation, conn.execute, "SELECT 1", prepare=True)
        pytest.raises(psycopg.errors.ProtocolViolation, conn.execute, "SELECT 1", prepare=True, binary=True)
    ...
    def _test_replication_slot(self, conn, expected_name, expected_type, expected_plugin):
        ((slot_name, slot_type, plugin),) = conn.execute("select slot_name, slot_type, plugin from pg_replication_slots")
...
class TestReplication(BaseTest):
    ...
    def test_logical_replication_connection(self, dsn, autocommit):
        with psycopg.connect(dsn, replication='database', autocommit=autocommit) as conn:
    ...
    @pytest.mark.parametrize('conn_type,kwargs', [
    ...
    def test_create_replication_slot(self, conn, slots, dsn, kwargs, conn_type):
        with (psycopg.connect(dsn, replication=conn_type, autocommit=True,
                              cursor_factory=ReplicationCursor) as rconn,
                rconn.cursor() as cur):

            slots.create_replication_slot(cur, 'test_slot', **kwargs)
            self._test_replication_slot(conn, 'test_slot', kwargs['type'], kwargs.get('output_plugin'))

    def test_create_replication_slot_fail(self, slots, dsn):
        with (psycopg.connect(dsn, replication='true', autocommit=True,
                              cursor_factory=ReplicationCursor) as rconn,
                rconn.cursor() as cur):

            pytest.raises(psycopg.errors.ObjectNotInPrerequisiteState, slots.create_replication_slot, cur, 'test_slot', type='logical', output_plugin='test_decoding')
    ...
    def test_drop_replication_slot(self, conn, slots, dsn, wait, kwargs, conn_type):
        with (psycopg.connect(dsn, replication=conn_type, autocommit=True,
            ...
            pytest.raises(psycopg.errors.UndefinedObject, cur.drop_replication_slot, 'test_slot')
    ...
    def test_start_replication(self, dsn, slots):
        with (psycopg.connect(dsn, replication='database', autocommit=True,
                              cursor_factory=ReplicationCursor) as conn,
            conn.cursor() as cur):

            with (pytest.raises(psycopg.errors.UndefinedObject),
                  cur.start_replication('test_slot', 'logical', '000/000')):
                pass

            slots.create_replication_slot(cur, 'test_slot', type='logical', output_plugin='test_decoding')
    ...
    def test_replication_cursor(self, dsn, slots):
        with (psycopg.connect(dsn, replication='database', autocommit=True,
                              cursor_factory=ReplicationCursor, row_factory=dict_row) as conn,
            conn.cursor() as cur):
...
class TestAsyncReplication(BaseTest):
    ...
    async def test_logical_replication_connection(self, dsn, autocommit):
        async with await psycopg.AsyncConnection.connect(dsn, replication='database', autocommit=autocommit) as aconn:
            await aconn.execute("SELECT 1")
            await self._test_async_extended_protocol(aconn)

    async def test_physical_replication_connection(self, dsn):
        async with await psycopg.AsyncConnection.connect(dsn, replication='true', autocommit=True) as aconn:
    ...
    async def test_create_replication_slot(self, conn, slots, dsn, kwargs, conn_type):
        async with (await psycopg.AsyncConnection.connect(dsn, replication=conn_type, autocommit=True, cursor_factory=AsyncReplicationCursor) as aconn,
    ...
    async def test_create_replication_slot_fail(self, slots, dsn):
        async with (await psycopg.AsyncConnection.connect(dsn, replication='true', autocommit=True, cursor_factory=AsyncReplicationCursor) as aconn,
                    aconn.cursor() as acur):

            with pytest.raises(psycopg.errors.ObjectNotInPrerequisiteState):
                await slots.create_async_replication_slot(acur, 'test_slot', type='logical', output_plugin='test_decoding')
    ...
    @pytest.mark.parametrize('wait', (True, False))
    async def test_drop_replication_slot(self, conn, slots, dsn, wait, kwargs, conn_type):
        async with (await psycopg.AsyncConnection.connect(dsn, replication=conn_type, autocommit=True, cursor_factory=AsyncReplicationCursor) as aconn,
                    aconn.cursor() as acur):
    ...
    async def test_start_replication(self, dsn, slots):
        async with (await psycopg.AsyncConnection.connect(dsn, replication='database', autocommit=True, cursor_factory=AsyncReplicationCursor) as aconn,
                    aconn.cursor() as acur):
